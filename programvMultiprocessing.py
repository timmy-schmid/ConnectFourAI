...
def evaluate_score_first_maximiser(max_depth, o_board, e_board, start_time):
    columns = [3, 2, 1, 4, 5, 6, 0] # Centred
    p = Pool(initializer=init_pool, initargs=(start_time, max_depth, o_board, e_board))
    xd = p.map(get_first_alpha_of_col, columns)
...
def connect_four(contents, turn):
    ...
    if use_IDS:
        ...
        while time.time() - start_t < 0.9:  # !!! Needs to be less than the main time_cutout.
            best_move = evaluate_score_first_maximiser(max_depth, own_board, enemy_board, start_time=start_t)
            evaluate_score.cache_clear()
            max_depth += 1
        #print(max_depth)
    else:
        best_move = evaluate_score_first_maximiser(6, own_board, enemy_board, start_time=start_t)
...
if __name__ == '__main__':
    print(connect_four(".ryyrry,.rryry.,.......,.......,.......,.......", "red"))
    # if len(sys.argv) <= 1:
    #     start_t = time.time()
    ...
    #     print(f"Best move: {best_move}. Time taken (s): {time.time() - start_t}")
    # else:
    #     board = sys.argv[1]
    #     player = sys.argv[2]
